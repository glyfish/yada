...
![](https://cdn.mathpix.com/cropped/2025_11_22_b612f70df9471f35da67g-2.jpg?height=558&width=1666&top_left_y=121&top_left_x=168)
FIG. 1. Scatter plot of the streamfunction $\psi$ versus the vorticity" $\omega$ at time $t=374$. The curve drawn through the plotted points is $\dot{c}^{-1} \sinh (|\beta| \psi)$. (For a "selectively decayed" state, there is a simple proportionality between $\psi$ and $\omega$.)

![](https://cdn.mathpix.com/cropped/2025_11_22_b612f70df9471f35da67g-2.jpg?height=1476&width=1777&top_left_y=960&top_left_x=160)
...
![](https://cdn.mathpix.com/cropped/2025_11_22_b612f70df9471f35da67g-3.jpg?height=1529&width=1788&top_left_y=94&top_left_x=139)
FIG. 4. Three-dimensional perspective plot of the computed streamfunction versus $x$ and $y$ at four different times.

could previously only be suggested. Namely, between $t \cong$ 292 and $t=374$, and presumably on to the large-scale en-ergy-decay time of $t \leqq 14000$, the streamfunction and vorticity obey, to a good approximation, a local relationship
...
where $\omega$ is the vorticity and $\psi$ is the streamfunction. The constants $c$ and $|\beta|$ are determined, at $t=374$, by a leastsquares fit, to be $c \cong 7.7$ and $\beta \leqq-2.1$. This is the necessary and sufficient condition that a sinh-Poisson equilibrium has been reached.
...
$$
\begin{equation*}
\frac{\partial \omega^{ \pm}}{\partial t}+\mathbf{v} \cdot \nabla \omega^{ \pm}=v \nabla^{2} \omega^{ \pm} \tag{2}
\end{equation*}
$$

then $\omega \equiv \omega^{+}-\omega^{-}$obeys the 2-D NS vorticity equation if $\mathbf{v}=\nabla \psi \times \widehat{e}_{z}$ and $\nabla^{2} \psi=-\omega^{+}+\omega^{--}=-\omega$. (The geometry is the conventional one with the activity lying in the $x-y$ plane.)
...
Thus, with these three constants of the motion, two rigorous ( $\int \omega^{ \pm} d x d y$ ) and one approximate [energy = (1/ 2) $\left.\int \omega \psi d x d y\right]$, the conservation conditions for the applicability of the maximum entropy theory are met. What is missing is a clear sense of the quasiergodic, or "mixing," behavior that the continuous, viscous Navier-Stokes fluid must exhibit. Some such behavior would appear to provide a basis for the entropy maximization which leads to the sinh-Poisson equation. The behavior is not familiar, and may require time to elucidate. But is appears to us as a fact of life which the theory of decaying 2-D Navier-Stokes turbulence will hereafter need to deal with.
